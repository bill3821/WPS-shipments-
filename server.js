...
// Geocode cache — avoids repeated lookups for same addresses
const geocodeCache = {};

// Open-Meteo geocoding (free, no key, no strict rate limit)
// Only works with city/place names — NOT street addresses
async function geocode(address) {
  ...
  // Open-Meteo only understands city names, so extract city from the address
  // "4700 McKinney Avenue, Dallas, Texas" → try "Dallas", then "Dallas, Texas", then full string
  const parts = address.split(',').map(s => s.trim()).filter(Boolean);
  const queries = [];

  // For multi-part addresses, try individual parts that look like city names (skip street numbers)
  for (const p of parts) {
    // Skip parts that start with numbers (street addresses like "4700 McKinney Ave")
    if (!/^\d/.test(p) && p.length > 1) {
      queries.push(p);
    }
  }
  // Also try combinations: "Dallas, Texas", "Toronto, ON"
  if (parts.length >= 2) queries.push(parts.slice(-2).join(', '));
  if (parts.length >= 3) queries.push(parts.slice(-3, -1).join(', '));
  // Full string as last resort
  queries.push(address);

  // Deduplicate
  const uniqueQueries = [...new Set(queries)];
  console.log(`[Geocode] Will try: ${JSON.stringify(uniqueQueries)}`);
  ...
  for (const q of uniqueQueries) {
    try {
      ...
      if (!res.ok) { console.error(`[Geocode] HTTP ${res.status} for "${q}"`); continue; }
      const data = await res.json();
      console.log(`[Geocode] Results for "${q}": ${data.results?.length || 0}`);
      if (data.results?.length > 0) {
        const r = data.results[0];
        const result = { lat: r.latitude, lon: r.longitude, name: r.name, state: r.admin1 || '', fullName: `${r.name}, ${r.admin1 || ''}${r.country ? ', ' + r.country : ''}` };
        console.log(`[Geocode] Found: ${result.fullName} (${result.lat}, ${result.lon})`);
        geocodeCache[address] = result;
        return result;
      }
    } catch (e) { console.error(`[Geocode] FETCH ERROR for "${q}":`, e.message, e.cause || ''); }
  }

  console.error(`[Geocode] Failed all attempts for "${address}"`);
  return null;
}
...
async function getRouteWeather(originAddr, destAddr) {
  const [originGeo, destGeo] = await Promise.all([geocode(originAddr), geocode(destAddr)]);
...
// HTTP SERVER — serves files from same directory as server.js (no public subfolder)
const server = http.createServer(async (req, res) => {
  ...
  // Serve static files (logo, etc) from same directory
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`  Files in app dir: ${fs.readdirSync(__dirname).join(', ')}\n`);
});
