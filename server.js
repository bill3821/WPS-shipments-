...
async function geocode(address) {
  ...
  // Extract just the city/state part for better geocoding results
  // Open-Meteo works best with city names, not full street addresses
  const parts = address.split(',').map(s => s.trim());
  // Try progressively shorter versions: full string, last 2 parts, last part
  const queries = [address];
  if (parts.length >= 2) queries.push(parts.slice(-2).join(', '));
  if (parts.length >= 3) queries.push(parts.slice(-3).join(', '));
  ...
  for (const q of queries) {
    try {
      ...
      if (!res.ok) { console.error(`[Geocode] Open-Meteo HTTP ${res.status}`); continue; }
    ...
    } catch (e) { console.error(`[Geocode] Error for "${q}":`, e.message); }
  }
